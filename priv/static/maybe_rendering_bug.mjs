...
function println_error(string3) {
  return console_error(string3);
}
function debug(term) {
  let _pipe = term;
...
function to_string6(size) {
  if (size instanceof Px) {
    let value4 = size[0];
    return append4(to_string4(value4), "px");
  } else if (size instanceof Pt) {
    let value4 = size[0];
    return append4(to_string4(value4), "pt");
  } else if (size instanceof Pct) {
    let value4 = size[0];
    return append4(to_string4(value4), "%");
  } else if (size instanceof Vh) {
    let value4 = size[0];
    return append4(to_string4(value4), "vh");
  } else if (size instanceof Vw) {
    let value4 = size[0];
    return append4(to_string4(value4), "vw");
  } else if (size instanceof Em) {
    let value4 = size[0];
    return append4(to_string4(value4), "em");
  } else if (size instanceof Rem) {
    let value4 = size[0];
    return append4(to_string4(value4), "rem");
  } else if (size instanceof Lh) {
    let value4 = size[0];
    return append4(to_string4(value4), "lh");
  } else if (size instanceof Rlh) {
    let value4 = size[0];
    return append4(to_string4(value4), "rlh");
  } else {
    let value4 = size[0];
    return append4(to_string4(value4), "ch");
  }
}
...
function width2(width3) {
  return property2("width", to_string6(width3));
}
...
function z_index(z_index2) {
  return property2("z-index", to_string2(z_index2));
}
...
function select2(debugger_, step2) {
  return debugger_.withFields({ selected_step: step2 });
}
function add_step(debugger_, model, msg) {
  let count = debugger_.count;
  let steps = debugger_.steps;
  let step2 = new Step(to_string2(count), model, msg);
  return debugger_.withFields({
    count: count + 1,
    steps: prepend(step2, steps)
  });
}
          ...
          return try$(
            ...
            (step2) => {
              ...
              let _pipe$22 = map(
                _pipe$13,
                (_capture) => {
                  return apply1(_capture, step2.model);
                }
              );
                  ...
                  return div(
                    ...
                    toList([
                      ...
                      div(
                        ...
                        toList([
                          text2(
                            to_string2(debugger_$1.count - 1) + " Steps"
                          )
                        ])
                      ),
                      button(
...
// build/dev/javascript/maybe_rendering_bug/maybe_rendering_bug_ffi.mjs
function document_get_element_by_id(id2) {
  let el2 = document.getElementById(id2);
  return el2 === null ? new Error2(`no element with id ${id2}`) : new Ok2(el2);
}
function canvas_get_context_2d(canvas2) {
  if (canvas2.getContext) {
    let ctx = canvas2.getContext("2d");
    return new Ok2(ctx);
  } else {
    return new Error2("canvas is not supported");
  }
}
function context_fill_rect(ctx, x, y, width3, height) {
  requestAnimationFrame(() => {
    ctx.fillRect(x, y, width3, height);
  });
  return ctx;
}

// build/dev/javascript/maybe_rendering_bug/maybe_rendering_bug.mjs
...
var UserChangedRectX = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var UserClickedDrawRectagle = class extends CustomType {
};
function default_model() {
  return new Model2(new CtxNeedsInit(), 10);
}
function init5(_) {
  return [default_model(), none()];
}
function view2(model) {
  let title = h1(toList([]), toList([text2("Hello, Canvas!")]));
  let canvas2 = canvas(
    toList([id("canvas"), width(500)])
  );
  let x_input = div(
    toList([]),
    toList([
  ...
  let draw_rect$1 = div(
    ...
    toList([
      button(
        toList([on_click(new UserClickedDrawRectagle())]),
        toList([text2("draw rectagle")])
      )
    ])
  );
  return div(toList([]), toList([title, canvas2, x_input, draw_rect$1]));
}
function get_canvas_context(id2) {
  return from2(
    (dispatch2) => {
      let ctx = try$(
        document_get_element_by_id(id2),
        (canvas2) => {
          return map3(
            canvas_get_context_2d(canvas2),
            (ctx2) => {
              return ctx2;
            }
          );
...
function draw_rect(ctx, x) {
  return from2(
    (dispatch2) => {
      if (ctx instanceof CtxReady) {
        let ctx$1 = ctx[0];
        let $ = context_fill_rect(ctx$1, x, 10, 50, 50);
        return dispatch2(new DoNothing());
      } else if (ctx instanceof CtxNeedsInit) {
        dispatch2(new GetCanvasContext());
        return dispatch2(new UserClickedDrawRectagle());
      } else {
        let error = ctx[0];
        println_error("there was some error: " + error);
        return dispatch2(new DoNothing());
      }
    }
  );
...
function update3(model, msg) {
  if (msg instanceof DoNothing) {
    ...
    return [model, get_canvas_context("canvas")];
  } else if (msg instanceof SetCanvasContext) {
    let ctx = msg[0];
    return [model.withFields({ ctx }), none()];
  } else if (msg instanceof UserChangedRectX) {
    let x = msg[0];
    return [model.withFields({ rect_x: x }), none()];
  } else {
    return [model, draw_rect(model.ctx, model.rect_x)];
  }
}
function main() {
  let $ = single2("main");
  ...
  let main$1 = $[0];
  let app = application(init5, update3, view2);
  let $1 = (() => {
    let _pipe = app;
